...
| `v4.pt.seg` | uint8 | 文档 id,跨文档隔离用:一个 512 块里可能有多篇文档 |

后两个文件源于「打包成定长块」这个选择。不打包则不需要它们,但会产生大量
padding。
...
**这一步做错不会报错**,整词掩码会静默退化成逐字掩码。验证方法(看词长
分布)见 [`docs/PRETRAIN.md`](../docs/PRETRAIN.md#验证产出)。
...
- **Wapic** —— CRF 分词器,标预训练的词边界。仓库里还自带 PD-1998 标注语料
  (`data/PeopleDaily1998.zip`),微调也需要。只做微调不必编译,用
  `bash prepare/install_deps.sh wapic-data` 只 clone

**编码行为变了,词表就和已发布模型对不上,而代码不会报错。** `make deps`
装完会自动比对,输出 `校验通过` 才算成功。

重建这两个依赖**之前**先跑 `python test/capture_baseline.py` 抓基线。顺序
反了,记录下来的就是改动之后的行为,失去比对意义。
...
两者都是 Make 的**文件目标**:产物存在就不重跑,切块耗时 4.6 小时。

## Makefile

已发布模型的完整命令行在里面,每个非默认参数旁有注释说明取值理由,
包括 `--alpha_pos 2.0` 这类。

两个规格用 `SIZE` 切换(`large` = 315M,`mid` = 165M,默认 mid)。预训练配方
相同,只差层数和 batch 切分,有效 batch 都是 4096。CSC 微调配方不同,原因见
[315M 用 5 epoch 严重欠训](../docs/WHY.md#315m-用-5-epoch-严重欠训)。
