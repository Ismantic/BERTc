...
BERTc 用个人开发者负担得起的成本,完成了一套从预训练到实际任务验证的中文
基础模型方案:从随机初始化开始,**一张 RTX 4090 约三天即可完成训练**;
模型结构、中英语料配比、训练参数都经过系统实验,沉淀为一套从数据
到发布均可复现的**中文 BERT 最佳实践**;效果也不只体现在预训练 Loss 上——
在中文分词、词性标注、命名实体识别和拼写纠错等实际任务中,BERTc 均有完整
评测,并在公开基准的统一口径下取得 **SOTA**。
...
- [Wapic](https://github.com/Ismantic/Wapic) —— CRF 中文分词器,标整词掩码的
  词边界。其还自带 PD-1998 标注语料,微调时要 clone 它(不必编译:
